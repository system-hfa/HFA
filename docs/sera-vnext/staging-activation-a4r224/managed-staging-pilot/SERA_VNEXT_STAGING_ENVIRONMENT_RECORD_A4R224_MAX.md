...
| Field | Value |
|---|---|
| Environment state | `LOCAL_WITH_REAL_DATABASE_LOOKUP_AND_INJECTED_HANDLER_CONTEXT` |
...
Selected: LOCAL_WITH_REAL_DATABASE_LOOKUP_AND_INJECTED_HANDLER_CONTEXT

Reason: No managed staging deployment (Vercel/Cloudflare) is provisioned for this repo. The real Supabase instance is available, a real enterprise admin record was verified via service-role query, and the handler was exercised through dependency injection. This is not the same as a real login session.
